...
import santo.com.crm.struct.ApiRestHeader;
...
public class ApiCtrlExceptionHandler {
    ...
    public ApiRestHeader handleBadRequest(Exception ex, WebRequest request) {
        ...
        } else if (ExceptionUtils.getStackTrace(ex).contains("uk_module_name")) {
            message = "Module with same name already exist.";
        } else if (ExceptionUtils.getStackTrace(ex).contains("es referida desde la tabla «tournament_team»")
                || ExceptionUtils.getStackTrace(ex).contains("is still referenced from table \"tournament_team\"")) {
            message = "Is not possible delete tournament because there are teams relationship with tournament.";
        } else if (ExceptionUtils.getStackTrace(ex).contains("\"match_detail\" violates foreign key constrain")) {
            message = "Can't delete player because this has events registered.";
        } else if (ExceptionUtils.getStackTrace(ex).contains("\"user\" violates foreign key constraint")) {
            message = "Can't delete user, restrictions are enabled.";
        }
    ...
    public ApiRestHeader handleBadRequest(DataIntegrityViolationException ex, WebRequest request) {
        ...
        if (ex.getMessage().contains("uniqueclienteid")) {
            msg = "Cliente ya  reregistrado.";
        } else if (ex.getMessage().contains("persona_genero_check")) {
            msg = "Opción de género no definido.";
        }
